...
const normalizeSkillInput = (skills) => {
  ...
  if (skills && typeof skills === "object") {
    ...
    if (Array.isArray(skills.skills)) {
      extracted.push(...skills.skills.map((skill) => String(skill || "").trim()).filter(Boolean));
    }
    if (skills.primary_skill || skills.primarySkill) {
      extracted.push(String(skills.primary_skill || skills.primarySkill).trim());
    }
    if (skills.secondary_skill || skills.secondarySkill) {
      extracted.push(String(skills.secondary_skill || skills.secondarySkill).trim());
    }
...
const mapSkillTextsToValues = (skills, options = []) => {
  const skillList = normalizeSkillInput(skills);
...
const CandidateBasicInfoStep = ({
    ...
    const parseAndMapResume = async () => {
      try {
        ...
        if (parsedData && typeof parsedData === "object") {
          const mappedData = parsedData.candidateForm || parsedData;
          console.log("Parsed Skills:", mappedData.skills);
          ...
          const parsedSkills = normalizeSkillInput(mappedData.skills || mappedData.primary_skill || mappedData.primarySkill || mappedData.secondary_skill || mappedData.secondarySkill || mappedData.skills_information || mappedData.skillSet);
          if (!normalizeText(formData.primarySkill) && parsedSkills.length > 0) {
            ...
            const mappedSkillValues = mapSkillTextsToValues(parsedSkills, primarySkillOptions);
            console.log("Parsed Skill Value:", {
              parsedSkills,
              mappedSkillValues,
              skillOptions: primarySkillOptions,
            });

            // If we matched option values, map them to primary/secondary.
            if (mappedSkillValues[0]) {
              updates.primarySkill = mappedSkillValues[0];
            }
            if (!normalizeText(formData.secondarySkill) && mappedSkillValues[1]) {
              updates.secondarySkill = mappedSkillValues[1];
            }

            // The visible Skill Set dropdown reads from formData.skills rows, not formData.primarySkill.
            ...
            const skillValuesForRows = mappedSkillValues.length > 0 ? mappedSkillValues : parsedSkills;
            if (!hasExistingPrimarySkill && skillValuesForRows.length > 0) {
              const skillDefaults = getSkillDefaultsFromExperience(updates.yearsExperience || formData.yearsExperience, Number(mappedData.totalExperience) || 0);
              updates.skills = skillValuesForRows.map((skillValue) => createSkillRow(skillValue, skillDefaults));
            }
          }

          // If the parser indicates the latest/only employment is an internship, avoid
          // treating it as recent experience. In that case do not populate current company/role
          // and mark candidate as fresher when appropriate.
          const employmentTypeRaw = String(mappedData.employmentType || "").toLowerCase();
          const currentDesignationRaw = String(mappedData.currentDesignation || "").toLowerCase();
          const currentCompanyRaw = String(mappedData.currentCompany || "").toLowerCase();
          const totalExpNumber = Number(mappedData.totalExperience) || 0;
          const lastIsIntern = employmentTypeRaw.includes("intern") || currentDesignationRaw.includes("intern") || currentCompanyRaw.includes("intern");

          if (lastIsIntern && totalExpNumber <= 1) {
            // treat as fresher: set candidateType to fresher and set experience bucket to '0-1'
            if (!normalizeText(formData.candidateType)) {
              updates.candidateType = "fresher";
            }
            if (!normalizeText(formData.yearsExperience)) {
              updates.yearsExperience = "0-1";
            }
            // do not populate currentCompanyName or jobTitleRole from internship
          } else {
            if (!normalizeText(formData.currentCompanyName) && normalizeText(mappedData.currentCompany)) {
              updates.currentCompanyName = mappedData.currentCompany;
            }
      ...
      try {
        ...
        const lastIsIntern = normalizedLower.includes("intern") || normalizedLower.includes("internship");
        const shouldTreatAsInternFresher = lastIsIntern && (yearsNumber === null || yearsNumber <= 1);

        if (shouldTreatAsInternFresher) {
          if (!normalizeText(formData.candidateType)) {
            updates.candidateType = "fresher";
          }
          if (!normalizeText(formData.yearsExperience)) {
          ...
          // Do not populate company/role from internship-only experience.
        } else {
  ...
  const handleSkillChange = (index, fieldName, value) => {
    if (fieldName === "primarySkill" || fieldName === "secondarySkill") {
