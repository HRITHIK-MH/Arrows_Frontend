...
const CandidateDocumentsStep = ({ formData, onChange, onSetStepFields }) => {
  ...
  const mapResumeToFields = async (file) => {
    ...
    try {
      ...
      parsed = result?.candidateForm || result;
    ...
    if (parsed) {
      ...
      if (!normalizeText(formData.primarySkill) && parsedSkills.length > 0) {
        ...
        const mappedSkillValues = mapSkillTextsToValues(parsedSkills, configSkillOptions);
        console.log("Parsed Skill Value:", {
          parsedSkills,
          mappedSkillValues,
          skillOptions: configSkillOptions,
        });

        if (mappedSkillValues.length > 0) {
        ...
        if (mappedSkillValues.length > 0 && (!Array.isArray(formData.skills)
          ...
          if (!hasExistingPrimarySkill) {
            updates.skills = mappedSkillValues.map((skillValue) => createSkillRow(skillValue, skillDefaults));
