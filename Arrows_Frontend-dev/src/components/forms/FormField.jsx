...
const FormField = ({
  ...
  const displayError =
    hasValue && typeof resolvedError === 'string' && resolvedError.toLowerCase().includes('required')
      ? ''
      : resolvedError;
  const isSkillSelect = name === 'primarySkill' || name === 'secondarySkill';

  if (isSkillSelect && (type === 'select' || type === 'searchable-select')) {
    console.log('Skill Options:', normalizedSelectOptions);
    console.log('Current Selected Skill Value:', {
      name,
      value,
      selectedOption: normalizedSelectOptions.find((opt) => String(opt.value) === String(value)) || null,
    });
  }
        ...
        <div className={`searchable-select-field${isDropdownOpen ? ' open' : ''}`} ref={dropdownRef}>
          <input
            ...
            value={
              isDropdownOpen
                ? searchTerm
                : (normalizedSelectOptions.find((opt) => String(opt.value) === String(value))?.label || String(value || ''))
            }
            onChange={(event) => {
          ...
          {isDropdownOpen && (
            <div id={`${name}-dropdown`} className="select-dropdown" role="listbox">
              {filteredOptions.length === 0 ? (
                ...
                (() => {
                  // If a value is selected but not present in filteredOptions, include it so user can see it
                  const hasCurrentValue = filteredOptions.some(opt => String(opt.value) === String(value));
                  const optionsToRender = hasCurrentValue ? filteredOptions : (value ? [{ key: '__current', value, label: String(value) }, ...filteredOptions] : filteredOptions);
        ...
        <div className={`select-field${isDropdownOpen ? ' open' : ''}`} ref={dropdownRef}>
          ...
          >
            <span className="select-value">
              {normalizedSelectOptions.find((opt) => String(opt.value) === String(value))?.label || (value ? String(value) : (placeholder || 'Select...'))}
            </span>
          ...
          {isDropdownOpen && (
            <div className="select-dropdown" role="listbox">
              {normalizedSelectOptions.length === 0 ? (
                ...
                (() => {
                  const hasCurrentValue = normalizedSelectOptions.some(opt => String(opt.value) === String(value));
                  const optionsToRender = hasCurrentValue ? normalizedSelectOptions : (value ? [{ key: '__current', value, label: String(value) }, ...normalizedSelectOptions] : normalizedSelectOptions);
                  return optionsToRender.map((option) => (
                    <button
                      key={option.key || option.value}
                      type="button"
