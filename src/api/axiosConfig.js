...
export const resolveApiBaseUrl = (serviceName = "") => {
  const backendConfigured = String(
  ...
  if (
    backendConfigured &&
    !/https?:\/\/api\.example\.com\/?$/i.test(backendConfigured)
  ) {
...
const getApiBaseUrl = () => {
  ...
  return /\/api$/i.test(apiUrl) ? apiUrl : `${apiUrl}/api`;
};
...
const normalizeServiceUrl = (serviceName, url = "") => {
  ...
  const shouldUseRootApi = normalizedService.toLowerCase() === "clientjob";
  const prefix =
    normalizedService && !shouldUseRootApi ? `/${normalizedService}` : "";
  ...
  const relativePath = `${prefix}/${path}`.replace(/\/+/g, "/");
...
export const createServiceApi = (serviceName) => {
  ...
  if (!name) {
    ...
    console.warn(
      "createServiceApi called without a service name — using root API",
    );
  }
  ...
  return {
    ...
    post: (url, data, config) =>
      API.post(normalizeServiceUrl(name, url), data, config),
    put: (url, data, config) =>
      API.put(normalizeServiceUrl(name, url), data, config),
    patch: (url, data, config) =>
      API.patch(normalizeServiceUrl(name, url), data, config),
    delete: (url, config) => API.delete(normalizeServiceUrl(name, url), config),
    request: (config) =>
      API.request({ ...config, url: normalizeServiceUrl(name, config?.url) }),
  };
};
...
const shouldRedirectToLogin = () => {
  ...
  return pathname !== "/login" && pathname !== "/sso/callback";
};
